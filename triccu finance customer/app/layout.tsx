...
import { SplashScreen } from '@/components/CustomerUI';
...
export default function CustomerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const hasShownSplash = sessionStorage.getItem('customer-splash-shown');
    const authSession = sessionStorage.getItem('customer-auth-session');
  ...
  const handleSplashComplete = () => {
    setLoading(false);
    sessionStorage.setItem('customer-splash-shown', 'true');
  };

  const handleLoginSuccess = () => {
    setIsAuthenticated(true);
    sessionStorage.setItem('customer-auth-session', 'true');
  };

  return (
    <>
      <AnimatePresence>
        {loading && !isOnboarding && <SplashScreen onComplete={handleSplashComplete} />}
      </AnimatePresence>

      <div className="flex flex-col min-h-[820px] bg-black">
        <div className={`flex-1 overflow-y-auto ${!isOnboarding ? 'pb-20' : ''}`}>
           {!loading && !isAuthenticated && !isOnboarding ? (
        ...
        <AnimatePresence>
          {!isOnboarding && isAuthenticated && (
            ...
            >
              <div className="glass-card border-white/10 rounded-[28px] px-5 py-2.5 flex justify-between items-center shadow-[0_12px_40px_rgba(0,0,0,0.5)] pointer-events-auto bg-black/45 backdrop-blur-xl">
