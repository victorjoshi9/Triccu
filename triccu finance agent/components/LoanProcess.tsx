...
export default function LoanProcess({ onClose, onSuccess }: LoanProcessProps) {
  // Step 0: Scan QR, Step 1: Info, Step 2: Eligibility, Step 3: NACH, Step 4: Success
  ...
  const [shopScanned, setShopScanned] = useState<string | null>(null);
  ...
  useEffect(() => {
    // Simulate QR Scanner opening automatically
    if (step === 0) {
       const timer = setTimeout(() => {
         setShopScanned('SH-8492');
         setStep(1); // Auto-progress to info after mock scan
       }, 2000);
        ...
        <div className="p-8">
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
               ...
               >
                  <div className="relative w-48 h-48 mb-6 border-2 border-emerald-500/50 rounded-3xl overflow-hidden shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                     <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center">
                        <QrCode className="w-16 h-16 text-emerald-500 opacity-20" />
                     </div>
                     <motion.div 
                       initial={{ y: 0 }}
                       animate={{ y: "100%" }}
                       transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}
                       className="absolute left-0 right-0 h-1 bg-emerald-400 shadow-[0_0_15px_rgba(16,185,129,1)]"
                     />
                  </div>
                  <h3 className="text-lg font-bold text-white mb-2">Scanning Shop QR...</h3>
                  <p className="text-xs text-white/50 text-center uppercase tracking-widest">Point camera at merchant's dynamic QR code</p>
               </motion.div>
            )}
            ...
            {step === 1 && (
              ...
              >
                {shopScanned && (
                  <div className="glass-card-light p-3 rounded-2xl flex items-center gap-3 border-emerald-500/30 text-emerald-400">
                    <CheckCircle2 className="w-5 h-5" />
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-400/60">Merchant Confirmed</p>
                      <p className="text-xs font-black">{shopScanned}</p>
                    </div>
                  </div>
                )}
                ...
                <button 
                  onClick={nextStep}
                  disabled={loading}
                  className="w-full py-5 green-gradient-bg rounded-[24px] font-black text-xs text-white uppercase tracking-[0.25em] shadow-2xl active:scale-95 transition-all mt-6 three-d-button neon-glow-green flex items-center justify-center gap-2"
                >
                  {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'VERIFY ELIGIBILITY'} {!loading && <ArrowRight className="w-5 h-5" />}
                </button>
              </motion.div>
            )}
            ...
            {step === 2 && (
              ...
              >
                 <div className="w-24 h-24 rounded-full bg-emerald-500/10 flex items-center justify-center mx-auto mb-6 relative">
                    <motion.div animate={{ rotate: 360 }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }} className="absolute inset-0 border-2 border-dashed border-emerald-500/50 rounded-full" />
                    <ShieldCheck className="w-10 h-10 text-emerald-400" />
                 </div>
                 <h3 className="text-xl font-bold text-white mb-2">Excellent Credit Score</h3>
                 <p className="text-sm text-white/60">Customer is eligible for zero-downpayment scheme with ₹1,00,000 limit.</p>
                 
                 <div className="glass-card-light p-4 rounded-2xl border-emerald-500/30 mt-6 flex flex-col gap-2 text-left">
                    <div className="flex justify-between items-center text-sm">
                       <span className="text-white/60">CIBIL Score</span>
                       <span className="text-emerald-400 font-bold">782</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                       <span className="text-white/60">KYC Status</span>
                       <span className="text-emerald-400 font-bold">Verified via Aadhaar</span>
                    </div>
                 </div>
